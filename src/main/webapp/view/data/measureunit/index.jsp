	...
	<script type="text/javascript">
		...
		$(function(){
			edg = $('#dt-units').edatagrid({
				...
				saveUrl:'<c:url value="/data/measureUnit/save.mvc"/>',
				updateUrl:'<c:url value="/data/measureUnit/edit.mvc"/>',
				destroyUrl:'<c:url value="/data/measureUnit/delete.mvc"/>'
			});
		});
	</script>
</head>
			...
			<table id="dt-units" fit="true" fitColumns="true" singleSelect="true" rownumbers="true" border="false">
				<thead>
					<tr>
						...
						<th field="unitName" width="200" editor="{type:'validatebox',options:{required:true}}">单位名称</th>
						<th field="description" width="300" editor="text">说明</th>
					</tr>
				</thead>
			</table>
		</div>
